...
var CreateRolesPage = React.createClass({
	...
	clear : function(appUuid){
		this.state.hints = {};
    	this.state.roles.roleName='';
    	this.state.roles.roleDesc='';
    	this.state.roles.roleStatus='1';
		this.state.roles.appUuid = appUuid;
	...
	onClickSave : function(){
		if(Validator.formValidator(this, this.state.roles)){
			...
			RolesActions.createAuthAppRoleGroup( this.state.roles );
		}
	},
	...
	render : function(){
		...
		return (
			<Modal visible={this.state.modal} width='540px' title="增加角数组" maskClosable={false} onOk={this.onClickSave} onCancel={this.toggle}
			  footer={[
			  	<div key="footerDiv" style={{display:'block', textAlign:'right'}}>
					<ServiceMsg ref='mxgBox' svcList={['auth-app-role-group/create']}/>
