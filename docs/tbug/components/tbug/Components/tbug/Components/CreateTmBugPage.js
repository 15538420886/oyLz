...
var CreateTmBugPage = React.createClass({
	...
	clear : function(InTmBug){
		this.state.hints = {};
		this.state.tmBug.bugName='';
		this.state.tmBug.bugDesp = '测试环境:测试目的: 重现步骤: 预期结果: 实际结果:'
		this.state.tmBug.bugPriorty='';
		this.state.tmBug.bugStage='';
		this.state.tmBug.bugSeverity='';
		this.state.tmBug.bugType='';
		this.state.tmBug.bugChance='';
		this.state.tmBug.sysId='';
		this.state.tmBug.findVer='';
		this.state.tmBug.mdlId='';
		this.state.tmBug.caseCode='';
		this.state.tmBug.bugResponsible='';
		...
		$.ajax({
			...
			var tmBugSelect = data.object;
			var obj = {};
			var obj2 = {};
			// 所属系统
			var systemModel = self.toArray(tmBugSelect.systemModel);
			...
			// 缺陷类型
			var bugType = self.toArray(tmBugSelect.bugType);

			// 严重程度
			var bugSeverity = self.toArray(tmBugSelect.bugSeverity);

			// 优先级
			var bugPriorty = self.toArray(tmBugSelect.bugPriorty);

			// 重现概率
			var bugChance = self.toArray(tmBugSelect.bugChance);

			// 测试阶段
			var bugStage = self.toArray(tmBugSelect.bugStage);

			// 用例编码
			var caseCode = self.toArray(tmBugSelect.bugStage);;

			// 处理人
			var bmUser = self.toArray(tmBugSelect.bmUser);
	...
	ClickSave: function(){
		this.state.tmBug.cmpId = window.loginData.compUser.corpUuid
		if (Common.formValidator(this, this.state.tmBug)) {
			this.state.tmBugSet.operation = '';
			this.setState({ loading: true });
			this.state.tmBug.uuid = this.state.tmBugSelect.uuid;
			TmBugActions.createTmBug(this.state.tmBug);
		}
	...
	handleOpenEnclosureWindow: function (e) {
		var tmBug = {
			uuid: this.state.tmBugSelect.uuid
		}
		if (tmBug != null) {
	...
	render : function(){
		...
		return (
			<Modal visible={this.state.modal} width='680px' title="增加缺陷管理信息" maskClosable={false} onCancel={this.toggle}
			  footer={[
			  	<div key="footerDiv" style={{display:'block', textAlign:'right'}}>
					...
					<Button key="btnOK" size="large" onClick={this.ClickSave} loading={this.state.loading}>保存</Button>
			...
			>
			   ...
			   <Row>
					<Col>
						<FormItem {...formItemLayout} label="缺陷名称" required={true} colon={true} help={hints.bugNameHint} validateStatus={hints.bugNameStatus}>
...
export default CreateTmBugPage;