...
var CreateStatStatDefinePage = React.createClass({
	...
	clear : function(InStatStatDefine){
		this.state.hints = {};
		this.state.statStatDefine.uuid='';
		this.state.statStatDefine.sttCode='';
		this.state.statStatDefine.sttName='';
		this.state.statStatDefine.isBegin='';
		this.state.statStatDefine.isFinal='';
		this.state.statStatDefine.disUse='';
		this.state.statStatDefine.InStatStatDefine = InStatStatDefine;
	...
	onClickSave : function(){
		this.state.statStatDefine.cmpId = this.props.InStatStatDefine.StatStatDefine.cmpId;
		this.state.statStatDefine.statMid = this.props.InStatStatDefine.StatStatDefine.uuid;
		this.state.statStatDefine.regUser = window.loginData.authUser.userId;
		if(Common.formValidator(this, this.state.statStatDefine)){
			this.state.statStatDefineSet.operation = '';
			this.setState({loading: true});
			StatStatDefineActions.createStatStatDefine( this.state.statStatDefine );
		}
	},
	...
	render : function(){
		...
		return (
			<Modal visible={this.state.modal} width='540px' title="增加状态机状态管理" maskClosable={false} onOk={this.onClickSave} onCancel={this.toggle}
			  footer={[
			  	<div key="footerDiv" style={{display:'block', textAlign:'right'}}>
					<ServiceMsg ref='mxgBox' svcList={['stat-stat-define/create']}/>
...
export default CreateStatStatDefinePage;
