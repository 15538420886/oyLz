...
import { Form, Modal, Button, Input, Select, Spin, AutoComplete,Row, Col,message,Table,Popconfirm ,Icon,Upload} from 'antd';
...
//              ResumeActions.getResumeByIdCode(data.recordSet[0].idCode);
    ...
    initPage: function (pid) {
     	
        this.state.hints = {};
        Utils.copyValue(pid, this.state.pid);
...
//          BmUserActions.retrieveTmCase(filter);
    ...
    onClickSave: function () {
        if (Validator.validator(this, this.state.userListSet)) {
            ...
            BmUserActions.createBmUser(this.state.userListSet);
             this.setState({
    ...
    render: function () {
        ...
        const formItemLayout2 = {
         ...
         const props = {
			name: 'file',
			multiple: true,
			action: 'http://10.10.10.201:8082/defect_s/bug-attach-upload/upload?uuid='+this.state.oid,
			onChange(info) {
