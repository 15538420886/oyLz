...
import RetrieveHotelRoom from './RetrieveHotelRoomPage';
...
var UpdateHotelPage = React.createClass({
	...
	componentDidMount : function(){
		...
		this.initPage( this.props.hotel );

	},

	initPage: function(hotel)
	{
		console.log(this);
		console.log(hotel);
		this.state.hints = {};
		Utils.copyValue(hotel, this.state.hotel);
	...
	onClickSave : function(){
		if(Common.formValidator(this, this.state.hotel)){
			...
			HotelActions.updateHotel( this.state.hotel );
		}
	},
		...
		return (
			<div style={{overflow:'hidden', height:'100%', paddingLeft: '4px'}}>
				<Tabs defaultActiveKey="2"  onChange={this.onTabChange} tabBarStyle={{paddingLeft: '16px', margin: '-36px 0 0'}} style={{width: '100%', height: '100%', padding: '36px 0 0'}}>
					...
					<TabPane tab="修改协议酒店信息" key="2" style={{width: '100%', height: '100%'}}>
						<div style={{padding:"8px 0 16px 8px", height: '100%',overflowY: 'auto'}}>
							<ServiceMsg ref='mxgBox' svcList={['hotel/update']}/>
					...
					<TabPane tab="房型" key="3" style={{width: '100%', height: '100%'}}>
						<div style={{padding:"8px 0 16px 8px", height: '100%',overflowY: 'auto'}}>
							<RetrieveHotelRoom  uuid={this.state.hotel.uuid}/>
						</div>
					</TabPane>
				</Tabs>
			</div>
