...
var CreateHotelRoomPage = React.createClass({
	...
	clear : function(filter){
		...
		this.state.hints = {};
		this.state.corpUuid = '';
		this.state.hotelRoom.hotelUuid='';
		this.state.hotelRoom.filter = filter;
		this.state.hotelRoom.roomType='';
		this.state.hotelRoom.breakfast='0';
		this.state.hotelRoom.priceLow='';
		this.state.hotelRoom.priceHigh='';
		this.state.hotelRoom.memo2='';
	...
	onClickSave : function(){
		if(Common.formValidator(this, this.state.hotelRoom)){
			...
			this.state.hotelRoom.hotelUuid = this.props.hotelUuid;
			this.state.hotelRoom.corpUuid = window.loginData.compUser.corpUuid;
			HotelRoomActions.createHotelRoom( this.state.hotelRoom );
		}
	},
	...
	render : function(){
		...
		return (
			<Modal visible={this.state.modal} width='540px' title="增加房间类型" maskClosable={false} onOk={this.onClickSave} onCancel={this.toggle}
			  footer={[
			  	<div key="footerDiv" style={{display:'block', textAlign:'right'}}>
					<ServiceMsg ref='mxgBox' svcList={['hotel_room/create']}/>
...
export default CreateHotelRoomPage;
