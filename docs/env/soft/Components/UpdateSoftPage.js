...
var UpdateSoftPage = React.createClass({
	...
	initPage: function(soft)
	{
		this.state.hints = {};
		Utils.copyValue(soft, this.state.soft);
	...
	onClickSave : function(){
		if(Validator.formValidator(this, this.state.soft)){
			...
			SoftActions.updateEnvSysSoft( this.state.soft );
		}
	},
	...
	render : function() {
		...
		return (
			<Modal visible={this.state.modal} width='540px' title="修改软件信息" maskClosable={false} onOk={this.onClickSave} onCancel={this.toggle}
			  footer={[
			  	<div key="footerDiv" style={{display:'block', textAlign:'right'}}>
					<ServiceMsg ref='mxgBox' svcList={['env-sys-soft/update']}/>
			...
			>
		   		<Form layout={layout}>
					...
					<FormItem {...formItemLayout} label="用途" required={false} colon={true} className={layoutItem} help={hints.softPurposeHint} validateStatus={hints.softPurposeStatus}>
						<Input type="text" name="softPurpose" id="softPurpose" value={this.state.soft.softPurpose } onChange={this.handleOnChange} />
					</FormItem>
