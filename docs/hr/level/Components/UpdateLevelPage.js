...
var UpdateLevelPage = React.createClass({
	...
	initPage: function(Level)
	{
		this.state.hints = {};
		Utils.copyValue(Level, this.state.Level);
	...
	onClickSave : function(){
		if(Validator.formValidator(this, this.state.Level)){
			...
			LevelActions.updateHrLevel( this.state.Level );
		}
	},
			...
			<Modal visible={this.state.modal} width='540px' title="修改管理信息" maskClosable={false} onOk={this.onClickSave} onCancel={this.toggle}
			  footer={[
			  	<div key="footerDiv" style={{display:'block', textAlign:'right'}}>
					<ServiceMsg ref='mxgBox' svcList={['hr-level/update']}/>
