...
var CreateWorkLogPage = React.createClass({
    ...
    clear: function () {
		this.state.hints = {};
		this.state.workLog.uuid='';
		this.state.workLog.corpUuid = window.loginData.compUser.corpUuid;
		this.state.workLog.userUuid = '';
		this.state.workLog.staffCode = '';
		this.state.workLog.applyName='';
        this.state.workLog.chgType ='转正';
        this.state.workLog.applyDate = '' + Common.getToday();
		this.state.workLog.befMemo='';
		this.state.workLog.aftMemo='';
		this.state.workLog.manager='';
		this.state.workLog.hrName='';
		this.state.workLog.approver='';
		this.state.workLog.effectDate='';
		this.state.workLog.chgReason='';
		//this.state.workLog.status='1';
    ...
    onSelectWorkLog:function(data){
        this.state.workLog.userUuid = data.uuid;
		this.state.workLog.staffCode = data.staffCode;
		this.state.workLog.perName = data.perName;
        this.state.workLog.deptName = data.deptName;
        this.state.workLog.deptUuid = data.deptUuid;
        // console.log('data', data);
        // console.log('this.state.workLog', this.state.workLog);
        if (!this.state.workLog.applyName) {
            this.state.workLog.applyName = data.perName;
        }
    ...
    onClickSave: function () {
        ...
        if (Common.formValidator(this, this.state.workLog)) {
            ...
            WorkLogActions.createHrWorkLog(this.state.workLog);
        }
	},
	...
	render : function() {
        ...
        var boo = this.state.workLog.userUuid? false : true ;
        ...
        if (chgType === '转部门') {
            this.state.workLog.befMemo = user.deptName;
            ...
            this.state.workLog.befMemo = user.baseCity;
            ...
            reasonTitle = '转正申请';
            this.state.workLog.aftMemo = '' + Common.getToday();
            ...
            chgFields.push(
                <FormItem {...formItemLayout} label="调后说明" required={true} colon={true} className={layoutItem} help={hints.aftMemoHint} validateStatus={hints.aftMemoStatus}>
	    ...
	    return (
			<div style={{overflow:'hidden', height:'100%', paddingLeft: '4px'}}>
				<Tabs defaultActiveKey="2"  onChange={this.onTabChange} tabBarStyle={{paddingLeft: '16px', margin: '-36px 0 0'}} style={{width: '100%', height: '100%', padding: '36px 0 0'}}>
					...
					<TabPane tab="增加工作变更信息" key="2" style={{width: '100%', height: '100%',overflowY:'auto'}}>
                        <div style={{padding:"8px 0 16px 8px", height: '100%',overflowY: 'auto'}}>
			            	<ServiceMsg ref='mxgBox' svcList={['hr_work_log/create']}/>
                            <SearchEmployee style={{padding:'10px 0 16px 32px', width:'600px'}} corpUuid={corpUuid} showError={this.showError} onSelectEmpLoyee={this.onSelectWorkLog}/>
								...
								<Row>
									...
									<Col span="12">
										<FormItem {...formItemLayout2} label="变更类型" required={false} colon={true} className={layoutItem} help={hints.chgTypeHint} validateStatus={hints.chgTypeStatus}>
                                            <DictSelect name="chgType" id="chgType" appName='HR系统' optName='工作变更类型' required={true} value={this.state.workLog.chgType} onSelect={this.handleOnSelected.bind(this, "chgType")}/>
										</FormItem>
									</Col>
								</Row>
								<Row>
...
module.exports = CreateWorkLogPage;
