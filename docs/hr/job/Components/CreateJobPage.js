...
var CreateJobPage = React.createClass({
	...
	clear : function(corpUuid, workUuid){
		this.state.hints = {};
		this.state.job.uuid='';
		this.state.job.corpUuid = corpUuid;
		this.state.job.workUuid = workUuid;
		this.state.job.jobCode='';
		this.state.job.jobName='';
		this.state.job.jobDesc='';
		this.state.job.unitTime='';
		this.state.job.unitPrice='';
		this.state.job.unitCost='';
		this.state.job.jobResponse='';
		this.state.job.jobQualify='';
		this.state.job.levelUuid='';
		this.state.job.levelUuid2='';
	...
	onClickSave : function(){
		if(Validator.formValidator(this, this.state.job)){
			...
			JobActions.createHrJob( this.state.job );
		}
	},
	...
	render : function(){
		...
		return (
			<Modal visible={this.state.modal} width='540px' title="增加岗位" maskClosable={false} onOk={this.onClickSave} onCancel={this.toggle}
			  footer={[
