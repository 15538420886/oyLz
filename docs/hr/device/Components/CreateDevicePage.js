...
var CreateDevicePage = React.createClass({
	...
	clear : function(corpUuid){
		...
		this.state.hints = {};
		this.state.device.uuid='';
		this.state.device.corpUuid = window.loginData.compUser.corpUuid;

		this.state.device.userUuid='';
		this.state.device.perName='';
		this.state.device.staffCode='';
		this.state.device.deptName='';
		this.state.device.devName='';
		this.state.device.devAllow='';
		this.state.device.devPrice='';
		this.state.device.effectDate='';
		this.state.device.expiryDate='';
		this.state.device.approver='';
		this.state.device.appDate='';
		this.state.device.appMemo='';
	...
	onSelectDevice:function(data){
        this.state.device.userUuid = data.uuid;
		this.state.device.staffCode = data.staffCode;
		this.state.device.perName = data.perName;
		this.state.device.deptName = data.deptName;
	...
	onClickSave : function(){
		...
		DeviceActions.createHrDevice( this.state.device );		
	},
	goBack:function(){
	...
	render : function(){
		...
		var boo = this.state.device.userUuid? false : true ;
		...
		return (
			<div style={{overflow:'hidden', height:'100%', paddingLeft: '4px'}}>
				<Tabs defaultActiveKey="2"  onChange={this.onTabChange} tabBarStyle={{paddingLeft: '16px', margin: '-36px 0 0'}} style={{width: '100%', height: '100%', padding: '36px 0 0'}}>
                    ...
                    <TabPane tab="增加设备补贴#" key="2" style={{width: '100%', height: '100%',overflowY:'auto'}}>
						<div style={{padding:"8px 0 16px 8px", height: '100%',overflowY: 'auto'}}>
							<ServiceMsg ref='mxgBox' svcList={['hr_device/create']}/>
							<SearchEmployee style={{padding:'10px 0 16px 32px', width:'600px'}} corpUuid={corpUuid} showError={this.showError} onSelectEmpLoyee={this.onSelectDevice}/>
				...
				<QuickFill ref='QuickFillWindow' self={this} object='device' />
			</div>
		);
	}
});
