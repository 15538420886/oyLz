...
var UpdateUnpaidLeavePage = React.createClass({
	...
	onClickSave : function(){
		if(Common.formValidator(this, this.state.leaveLog)){
            ...
            console.log("this.state.leaveLog",this.state.leaveLog);
			LeaveLogActions.updateHrLeaveLogReg( this.state.leaveLog );
		}
	},
	...
	render : function() {
		...
		return (
			<Modal visible={this.state.modal} width='720px' title="修改无薪休假" maskClosable={false} onOk={this.onClickSave} onCancel={this.toggle}
			  footer={[
...
export default UpdateUnpaidLeavePage;