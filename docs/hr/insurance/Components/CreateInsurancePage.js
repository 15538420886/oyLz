...
var CreateInsurancePage = React.createClass({
	...
	componentDidMount : function(){
        this.state.validRules = [
            { id: 'insuName', desc: '社保名称', required: true, max: '64' },
            { id: 'health', desc: '医疗保险', required: false, max: '16' },
            { id: 'pension', desc: '养老保险', required: false, max: '16' },
            { id: 'maternity', desc: '生育保险', required: false, max: '16' },
            { id: 'injury', desc: '工伤保险', required: false, max: '16' },
            { id: 'jobless', desc: '失业保险', required: false, max: '16' },
            { id: 'accumulation', desc: '公积金', required: false, max: '16' },
	...
	clear : function(corpUuid){
		...
		this.state.hints = {};
		this.state.insurance.uuid='';
		this.state.insurance.corpUuid = corpUuid;

		this.state.insurance.insuName='';
		this.state.insurance.accumulation='';
		this.state.insurance.health='';
		this.state.insurance.pension='';
		this.state.insurance.effectDate='';
	...
	onClickSave : function(){
		if(Validator.formValidator(this, this.state.insurance)){
			...
			InsuranceActions.createHrInsurance( this.state.insurance );
		}
	},
	...
	render : function(){
		...
		return (
			<Modal visible={this.state.modal} width='540px' title="增加社保参数管理#" maskClosable={false} onOk={this.onClickSave} onCancel={this.beforeClose}
			  footer={[
			  	<div key="footerDiv" style={{display:'block', textAlign:'right'}}>
					<ServiceMsg ref='mxgBox' svcList={['hr_insurance/create']}/>
			...
			>
				...
				<QuickFill ref='QuickFillWindow' self={this} object='insurance' />
			</Modal>
		);
	}
});
