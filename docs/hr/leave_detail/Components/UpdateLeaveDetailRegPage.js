...
var UpdateLeaveDetailRegPage = React.createClass({
	...
	initPage: function(detail)
	{
        ...
        // 加载假日数量
        this.state.leaveLoading = true;
		this.state.leave = {};
        this.state.leave2 = {};
        LeaveDetailActions.retrieveLeave(detail.staffCode);
	},
	...
	onClickSave : function(){
		if(Common.formValidator(this, this.state.detail)){
			...
			LeaveDetailActions.updateHrLeaveDetailRegWithLeave( obj );
		}
	},
		...
		return (
			<Modal visible={this.state.modal} width='540px' title="修改假日明细信息" maskClosable={false} onOk={this.onClickSave} onCancel={this.toggle}
			  footer={[
			  	<div key="footerDiv" style={{display:'block', textAlign:'right'}}>
					<ServiceMsg ref='mxgBox' svcList={['hr-leave-detail/update','hr-leave-detail/retrieveLeave']}/>
			...
			>
                <Form layout={layout}>
					...
					<FormItem {...formItemLayout3} style={{margin:'24px 0 10px 0'}} label="调整前假日" required={false} colon={true} className={layoutItem}>
                        {fields.bfFields}
					</FormItem>
					<FormItem {...formItemLayout3} label="调整后假日" required={true} colon={true} className={layoutItem} help={hints.remnantHint} validateStatus={hints.remnantStatus}>
                        {fields.afFields}
					</FormItem>
				</Form>
			</Modal>
...
export default UpdateLeaveDetailRegPage;
