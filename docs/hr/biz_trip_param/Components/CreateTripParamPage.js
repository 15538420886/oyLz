...
var CreateTripParamPage = React.createClass({
	...
	componentDidMount : function(){
		this.state.validRules = [
			...
			{id: 'allowance', desc:'出差补贴', required: false, max: '16'},
	...
	clear : function(corpUuid,tripUuid){
		this.state.hints = {};
		this.state.tripParam.uuid='';
		this.state.tripParam.tripUuid = tripUuid;
		this.state.tripParam.corpUuid = corpUuid;
		this.state.tripParam.cityType='';
		this.state.tripParam.allowance='';
		this.state.tripParam.traffic='';
		this.state.tripParam.taxiPort='';
		this.state.tripParam.taxiCity='';
		this.state.tripParam.hotelMemo='';
		this.state.tripParam.foodMemo='';
	...
	onClickSave : function(){
		if(Validator.formValidator(this, this.state.tripParam)){
			...
			TripParamActions.createHrBizTripParam( this.state.tripParam );
		}
	},
	...
	render : function(){
		...
		return (
			<Modal visible={this.state.modal} width='540px' title="增加额度" maskClosable={false} onOk={this.onClickSave} onCancel={this.toggle}
			  footer={[
			  	<div key="footerDiv" style={{display:'block', textAlign:'right'}}>
					<ServiceMsg ref='mxgBox' svcList={['hr-biz-trip-param/create']}/>
...
export default CreateTripParamPage;
