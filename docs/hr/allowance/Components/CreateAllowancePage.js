...
var CreateAllowancePage = React.createClass({
	...
	clear : function(corpUuid){
		...
		this.state.hints = {};
		this.state.allowance.uuid='';
		this.state.allowance.corpUuid = corpUuid;
		this.state.allowance.allowName='';
		this.state.allowance.allowDesc='';
		this.state.allowance.effectDate='';
		this.state.allowance.traffic='';
		this.state.allowance.phone='';
		this.state.allowance.food='';
		this.state.allowance.clothing='';
		this.state.allowance.highTemp='';
		this.state.allowance.outDoor='';
	...
	onClickSave : function(){
		if(Validator.formValidator(this, this.state.allowance)){
			...
			AllowanceActions.createHrAllowance( this.state.allowance );
		}
	},
	...
	render : function(){
		...
		return (
            <Modal visible={this.state.modal} width='540px' title="增加补贴级别#" maskClosable={false} onOk={this.onClickSave} onCancel={this.beforeClose}
			  footer={[
			  	<div key="footerDiv" style={{display:'block', textAlign:'right'}}>
					<ServiceMsg ref='mxgBox' svcList={['hr_allowance/create']}/>
			...
			>
				...
				<QuickFill ref='QuickFillWindow' self={this} object='allowance' />
			</Modal>
		);
	}
});

export default CreateAllowancePage;
