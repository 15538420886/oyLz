...
var CreateWifiPage = React.createClass({
	...
	clear : function(roomUuid){
		this.state.hints = {};
		this.state.wifi.uuid ='';
		this.state.wifi.roomUuid = roomUuid;
		this.state.wifi.wifiId ='';
		this.state.wifi.wifiName ='';
	...
	onClickSave : function(){
		if(Validator.formValidator(this, this.state.wifi)){
			...
			WifiActions.createHrWifi( this.state.wifi );
		}
	},
	...
	render : function(){
	  	if(this.state.modal && this.state.wifiSet.operation === 'create'){
		...
		if( this.state.loading ){
		    if(this.state.wifiSet.operation === 'create'){
		        this.state.loading = false;
		    }
		}
		...
		return (
			<Modal visible={this.state.modal} width='540px' title="增加WIFI" maskClosable={false} onOk={this.onClickSave} onCancel={this.toggle}
			  footer={[
			  	<div key="footerDiv" style={{display:'block', textAlign:'right'}}>
					<ServiceMsg ref='mxgBox' svcList={['hr-wifi/create']}/>
...
ReactMixin.onClass(CreateWifiPage, Reflux.connect(WifiStore, 'wifiSet'));
