...
var UpdateInnRoomPage = React.createClass({
	...
	initPage: function(room)
	{
		this.state.hints = {};
		Utils.copyValue(room, this.state.room);
	...
	onClickSave : function(){
		...
		if(isValid){
			...
			InnRoomActions.updateHrRoom( this.state.room );
		}
		else{
			this.setState({
	...
	render : function() {
	    ...
	    return (
	        <Modal visible={this.state.modal} width='540px' title="修改房间信息" maskClosable={false} onOk={this.onClickSave} onCancel={this.toggle}
	          footer={[
	          	<div key="footerDiv" style={{display:'block', textAlign:'right'}}>
			        <ServiceMsg ref='mxgBox' svcList={['inn-room/update']}/>
...
export default UpdateInnRoomPage;
