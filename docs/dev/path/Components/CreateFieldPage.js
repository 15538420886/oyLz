...
var CreateFieldPage = React.createClass({
    ...
    onServiceComplete: function(data) {
        if (data.operation === 'updateFields') {
            if (data.errMsg === '') {
                // 成功，关闭窗口
                this.setState({
                    modal: false
    ...
    clear: function (appUuid, puuid) {
        this.state.hints = {};
        this.state.field.id = '';
        this.state.field.desc = '';
        this.state.field.required = '0';
        this.state.field.max = '';
        this.state.field.min = '';
        this.state.field.datatype = '';
        this.state.field.pattern = '';
        this.state.field.patternPrompt = '';
        this.state.field.validator = '';
        this.state.field.dafValue = '';
        this.state.field.type = 'text';
        this.state.field.opts = '';
        this.state.field.showCode = '0';
    ...
    onClickSave: function () {
        if (Validator.formValidator(this, this.state.field)) {
            ...
            if (errMsg !== null) {
                const modal = Modal['warning']({ title: '警告', content: errMsg, width: 400 });
                return;
            }
    ...
    render: function () {
        ...
        return (
            <Modal visible={this.state.modal} width='540px' title="增加字段" maskClosable={false} onOk={this.onClickSave} style={{marginTop:'-15px'}} onCancel={this.toggle}
                footer={[
                    ...
                    <Row>
                        <Col span="24">
                            <FormItem {...formItemLayout} label="名称" required={true} colon={true} className={layoutItem} help={hints.idHint} validateStatus={hints.idStatus}>
                                <Input type="text" name="id" id="id" value={this.state.field.id} onChange={this.handleOnChange} />
                            </FormItem>
                        </Col>
                    </Row>
                    <Row>
                    ...
                    <Row>
                        <Col span="24">
                            <FormItem {...formItemLayout} label="检索字段" colon={true} className={layoutItem} help={hints.optsHint} validateStatus={hints.optsStatus}>
                                <Search id='opts' name='opts' value={this.state.field.opts} placeholder='请进行字段检索' onSearch={this.onOptsSelect} readOnly={true}/>
                            </FormItem>
                        </Col>
                    </Row>
                    <Row>
