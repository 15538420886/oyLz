...
var CreateParamDefPage = React.createClass({
  ...
  componentDidMount : function() {
      this.state.validRules = [
          ...
          {id: 'valueSet', desc:'取值范围', max: 64}
      ];
  },

  clear : function(groupUuid, appUuid) {
    this.state.hints = {};
    this.state.paramdef.groupUuid = groupUuid;
    this.state.paramdef.appUuid = appUuid;
    this.state.paramdef.paraName='';
    this.state.paramdef.paraDesc='';
    this.state.paramdef.valueSet='';
    this.state.paramdef.paraStatus='1';
    this.state.paramdef.operation = '';
  ...
  onClickSave : function() {
    if(Validator.formValidator( this, this.state.paramdef )){
        ...
        ParamDefActions.createParamDef( this.state.paramdef );
      }
  },
  ...
  render : function() {
    ...
    return (
      <Modal visible={this.state.modal} width='540px' title="增加参数" maskClosable={false} onOk={this.onClickSave} onCancel={this.toggle}
        footer={[
          <div key="footerDiv" style={{display:'block', textAlign:'right'}}>
            <ServiceMsg ref='mxgBox' svcList={['para_def/create']} />
...
export default CreateParamDefPage;
