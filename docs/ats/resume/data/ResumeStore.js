...
var ResumeStore = Reflux.createStore({
    ...
    // 上传简历文件
    onCreateResumeUpload:function(data, file){
         var url = this.getServiceUrl('resume/create-resume');
    ...
    onUpdateResume: function(filter) {
        var self = this; 
        ...
        console.log("resume",filter)
         Utils.doUpdateService(url, filter).then(function(result) {
            if(result.errCode==null || result.errCode=='' || result.errCode=='000000'){
                ...
                // self.object = result.object;
                //做页面处理 recordSet 下的对象
                var resumeUuid=filter.filter;
                var resumeIdx = Utils.findRecord( self, resumeUuid );
                if(resumeIdx > -1){
                    //找到简历
                    
                    self.recordSet[resumeIdx] = result.object;
                }
