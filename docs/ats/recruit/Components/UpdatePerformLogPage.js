...
var UpdatePerformLogPage = React.createClass({
	...
	initPage: function(performLog)
	{
		this.state.hints = {};
		Utils.copyValue(performLog, this.state.performLog);	
	...
	onClickSave : function(){
		if(Common.formValidator(this, this.state.performLog)){
			...
			PerformLogActions.updatePerformLog( filterObj );
		}
	},
	...
	render : function() {
		...
		return (
			<Modal visible={this.state.modal} width='540px' title="修改执行情况" maskClosable={false} onOk={this.onClickSave} onCancel={this.toggle}
			  footer={[
			  	<div key="footerDiv" style={{display:'block', textAlign:'right'}}>
					<ServiceMsg ref='mxgBox' svcList={['perform-Log/update']}/>
...
export default UpdatePerformLogPage;