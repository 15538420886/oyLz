...
var CreateJobSalaryPage = React.createClass({
	...
	clear : function(filter){
		this.state.hints = {};
		this.state.jobSalary.jobLocal='';
		this.state.jobSalary.jobSalary='';
		this.state.jobSalary.jobNature='全职';
	...
	onClickSave : function(){
		if(Common.formValidator(this, this.state.jobSalary)){
			...
			JobSalaryActions.createStdJob(filterObj);
		}
	},
	...
	render : function(){
		...
		return (
			<Modal visible={this.state.modal} width='540px' title="增加工资信息" maskClosable={false} onOk={this.onClickSave} onCancel={this.toggle}
			  footer={[
			...
			>
		   		<Form layout={layout}>
					<FormItem {...formItemLayout} className={layoutItem} label='地区' required={true} colon={true} help={hints.jobLocalHint} validateStatus={hints.jobLocalStatus}>
						<Input type='text' name='jobLocal' id='jobLocal' value={this.state.jobSalary.jobLocal} onChange={this.handleOnChange} />
					</FormItem>
...
export default CreateJobSalaryPage;