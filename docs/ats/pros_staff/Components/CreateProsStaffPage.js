...
import ProjContext from '../../../proj/ProjContext';
...
var DeptStore = require('../../../hr/dept/data/DeptStore.js');
var DeptActions = require('../../../hr/dept/action/DeptActions');

var CreateProsStaffPage = React.createClass({
    ...
    mixins: [Reflux.listenTo(ProsStaffStore, "onServiceComplete"), 
    Reflux.listenTo(DeptStore, "onDeptComplete"),ModalForm('prosStaff')],
    ...
    componentDidMount: function () {
        this.state.validRules = [
            ...
            { id: 'deptUuid', desc: '入职部门', required: true, max: 64, },
    ...
    afterChange: function (id, value) {
        if (id === 'idCode') {
            if (this.state.prosStaff.idType === '身份证' && value.length === 18) {
                var birth = value.substr(6, 8);
                var flag = value.substr(16, 1);
                this.state.prosStaff.birthDate = birth;
                this.state.prosStaff.gender = (flag % 2 == 0) ? 'woman' : 'man';
            }
        }
        else if (id === 'recruitUuid') {
            var recruitArr = this.refs.recruitUuid.getRecruitNode(value);
            if (recruitArr) {
                ...
                var deptSet = this.state.deptSet;
                for(var i =0;i<deptSet.recordSet.length;i++) {
                    var item = deptSet.recordSet[i];
                    if(item.deptName === recruitArr.applyDept && item.uuid!=='') {
                        this.state.prosStaff.deptUuid = item.uuid;
                        break;
                    } else {
                        this.state.prosStaff.deptUuid = '';
                    }
    ...
    onClickSave: function () {
        ...
        if (Common.formValidator(this, prosStaff)) {
            this.setState({ loading: true });
            prosStaff.corpUuid = window.loginData.compUser.corpUuid;
            prosStaff.enterStatus = '入职';
            prosStaff.enterDate = prosStaff.expectDate;
            ProsStaffActions.createProsStaff(prosStaff);
        }
        ...
        if(hrExeNode.deptName === null || hrExeNode.deptName === '' || hrExeNode.deptName === hrExeNode.deptCode){
            prosStaff.deptName = hrExeNode.deptName;
        }
        else{
            prosStaff.deptName = hrExeNode.deptCode+'('+hrExeNode.deptName+')';
        }   
    },
    //返回
    goBack: function () {
        ...
        return (
            <div style={{ overflow: 'hidden', height: '100%', paddingLeft: '4px' }}>
                <Tabs defaultActiveKey="2" onChange={this.onTabChange} tabBarStyle={{ paddingLeft: '16px', margin: '-36px 0 0' }} style={{ width: '100%', height: '100%', padding: '36px 0 0' }}>
                    ...
                    <TabPane tab="增加待入职人员信息" key="2" style={{ width: '100%', height: '100%' }}>
                        <div style={{ padding: "24px 0 16px 8px", height: '100%', overflowY: 'auto' }}>
                            <ServiceMsg ref='mxgBox' svcList={['pros-staff/create']} />
                            <Form layout={layout} style={{ width: '660px' }}>
                                ...
                                <Row>
                                    <Col span="12">
                                        <FormItem {...formItemLayout2} className={layoutItem} label='电话' required={true} colon={true} help={hints.phoneHint} validateStatus={hints.phoneStatus}>
                                            <Input type='text' name='phone' id='phone' value={this.state.prosStaff.phone} onChange={this.handleOnChange} />
                                        </FormItem>
                                    </Col>
                                    <Col span="12">
                                        <FormItem {...formItemLayout2} className={layoutItem} label='电子邮箱' required={true} colon={true} help={hints.emailHint} validateStatus={hints.emailStatus}>
                                ...
                                <Row>
                                    ...
                                    <Col span="12">
                                        <FormItem {...formItemLayout2} className={layoutItem} label='专业' colon={true} help={hints.professionHint} validateStatus={hints.professionStatus}>
                                            <AutoInput name='profession' id='profession' paramName='专业' value={this.state.prosStaff.profession} onChange={this.handleOnSelected.bind(this, "profession")} />
                                        </FormItem>
                                    </Col>
                                </Row>
                                <Row>
                                    <Col span="12">
                                        <FormItem {...formItemLayout2} label="工龄" className={layoutItem} colon={true} help={hints.workYearsHint} validateStatus={hints.workYearsStatus}>
                                            <Col span='11'>
                                                <Input.Group compact>
                                ...
                                <Row>
                                    <Col span="12">
                                        <FormItem {...formItemLayout2} className={layoutItem} label='属地' colon={true} help={hints.baseCityHint} validateStatus={hints.baseCityStatus}>
                                            <AutoInput name='baseCity' id='baseCity' paramName='城市' value={this.state.prosStaff.baseCity} onChange={this.handleOnSelected.bind(this, "baseCity")} />
                                        </FormItem>
                                    </Col>
                                    <Col span="12">
                                        <FormItem {...formItemLayout2} className={layoutItem} label='入职部门' required={true} colon={true} help={hints.deptUuidHint} validateStatus={hints.deptUuidStatus}>
                                        <DeptTreeSelect ref="DeptTreeName" name="deptUuid" id="deptUuid" value={this.state.prosStaff.deptUuid } onSelect={this.handleOnSelected.bind(this,'deptUuid')} />
                                        </FormItem>
                                    </Col>
                                </Row>
                                <Row>
...
export default CreateProsStaffPage;
