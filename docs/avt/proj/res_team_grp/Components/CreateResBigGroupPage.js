...
var CreateResBigGroupPage = React.createClass({
	...
	clear : function(){
		this.state.hints = {};
		this.state.teamGrp.puuid= this.props.puuid;
		this.state.teamGrp.corpUuid= window.loginData.compUser.corpUuid;
		this.state.teamGrp.grpCode= this.props.grpCode+"."+ ' ';
		this.state.teamGrp.poolUuid= this.props.poolUuid;
		this.state.teamGrp.teamUuid= this.props.teamUuid;
		this.state.teamGrp.grpName='';
		this.state.teamGrp.grpLevel='小组';
		this.state.teamGrp.baseCity='';
		this.state.teamGrp.pmCode='';
		this.state.teamGrp.pmName='';
		this.state.teamGrp.grpDesc='';
	...
	onClickSave : function(){
		if(Common.formValidator(this, this.state.teamGrp)){
			...
			ResBigGroupActions.createResTeamGrp( this.state.teamGrp );
		}
	},
	...
	render : function(){
		...
		return (
			<Modal visible={this.state.modal} width='580px' title="增加小组" maskClosable={false} onOk={this.onClickSave} onCancel={this.toggle}
			  footer={[
...
export default CreateResBigGroupPage;
