...
var SalaryItemStore = require('../../../../hr/salary_item/data/SalaryItemStore');
var SalaryItemActions = require('../../../../hr/salary_item/action/SalaryItemActions');
			...
			objList[i].map((item,index)=>{
				if(firstCol === null){
					firstCol = <Col span="12">
							<FormItem {...formItemLayout2} label={item.itemName} className={layoutItem} >
		                		<Input style={{zIndex:"2"}} type="text" name={item.uuid} id={item.uuid} value={this.state.salaryBody? this.state.salaryBody[item.itemCode]:'' }  readOnly={true}/>
		            		</FormItem>
						</Col>;
				}
				else{
