...
var UpdateUnpaidLeaveApplyPage = React.createClass({
    ...
    onClickSave: function () {
        if (Common.formValidator(this, this.state.leaveApply)) {
            ...
            // console.log('this.state.leaveApply:::', this.state.leaveApply);
    ...
    render: function () {
        ...
        var divWidth = 220 * cardList.length + 10 + 'px';
        return (
            <Modal visible={this.state.modal} width='640px' title="修改无薪假" maskClosable={false} onOk={this.onClickSave} onCancel={this.toggle}
                footer={[
                    ...
                    <Row>
                        <Col span={12}>
                            <FormItem {...formItemLayout2} label="假期类型" required={false} colon={true} className={layoutItem} help={hints.leaveTypeHint} validateStatus={hints.leaveTypeStatus}>
                                <Input type="text" name="leaveType" id="leaveType" value={this.state.leaveApply.leaveType} disabled={true} />
                            </FormItem>
                        </Col>
                    </Row>
                    <Row>
