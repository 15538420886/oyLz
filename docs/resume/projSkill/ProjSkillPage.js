...
import CreateProjSkillPage from './Components/CreateProjSkillPage';

var ProjSkillPage = React.createClass({
	...
	onClickUpdate:function(projSkill,event){
		if(projSkill != null){
			if(typeof(this.refs.createWindow) != 'undefined'){
				this.refs.createWindow.initPage(projSkill);
			}
		}
	},
	onClickDelete:function(projSkill,event){
		Modal.confirm({
			title: '删除确认',
			content: '是否删除选中的记录 【'+projSkill.uuid+'】',
			okText: '确定',
			cancelText: '取消',
			onOk: this.onClickDelete2.bind(this, projSkill)
		});
	},
	onClickDelete2 : function(projSkill){
		this.setState({loading: true});
		ResumeActions.delProjSkill(projSkill.uuid);
	},
	render : function() {
		var recordSet=this.state.projSkillSet.person.projSkillList;
		const columns = [
		{
		    title: '技能名称',
		    dataIndex: 'skType',
        ...
        },
       {
		...
		return (
			<div className='resume-page'>
				<ServiceMsg ref='mxgBox' svcList={['projSkillList/remove']}/>
                ...
                <CreateProjSkillPage ref="createWindow" />
			</div>
		);
	}
});

module.exports = ProjSkillPage;
