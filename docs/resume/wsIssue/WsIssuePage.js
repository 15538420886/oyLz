...
import CreateWsIssuePage from './Components/CreateWsIssuePage';

var WsIssuePage = React.createClass({
	...
	onClickUpdate:function(wsIssue,event){
		if(wsIssue != null){
			if(typeof(this.refs.createWindow) != 'undefined'){
				this.refs.createWindow.initPage(wsIssue);
			}
		}
	},
	onClickDelete:function(wsIssue,event){
		Modal.confirm({
			title: '删除确认',
			content: '是否删除选中的记录 【'+wsIssue.uuid+'】',
			okText: '确定',
			cancelText: '取消',
			onOk: this.onClickDelete2.bind(this, wsIssue)
		});
	},
	onClickDelete2 : function(wsIssue){
		this.setState({loading: true});
		ResumeActions.delWsIssue(wsIssue.uuid);
	},
	render : function() {
		var recordSet=this.state.wsIssueSet.person.wsIssueList;
		const columns = [
		{
			title: '作品名称',
			dataIndex: 'title',
		...
		return (
			<div className='resume-page'>
				<ServiceMsg ref='mxgBox' svcList={['wsIssueList/remove']}/>
                ...
                <CreateWsIssuePage ref="createWindow" />
			</div>
		);
	}
});

module.exports = WsIssuePage;
