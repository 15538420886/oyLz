...
import CreateSoHonorPage from './Components/CreateSoHonorPage';

var SoHonorPage = React.createClass({
	...
	onClickUpdate:function(soHonor,event){
		if(soHonor != null){
			if(typeof(this.refs.createWindow) != 'undefined'){
				this.refs.createWindow.initPage(soHonor);
			}
		}
	},
	onClickDelete:function(soHonor,event){
		Modal.confirm({
			title: '删除确认',
			content: '是否删除选中的记录 【'+soHonor.uuid+'】',
			okText: '确定',
			cancelText: '取消',
			onOk: this.onClickDelete2.bind(this, soHonor)
		});
	},
	onClickDelete2 : function(soHonor){
		this.setState({loading: true});
		ResumeActions.delSoHonor(soHonor.uuid);
	},
	render : function() {
		var recordSet=this.state.soHonorSet.person.soHonorList;
		const columns = [
		{
			title: '奖项名称',
			dataIndex: 'hoName',
		...
		return (
			<div className='resume-page'>
				<ServiceMsg ref='mxgBox' svcList={['soHonorList/remove']}/>
                ...
                <CreateSoHonorPage ref="createWindow" />
			</div>
		);
	}
});

module.exports = SoHonorPage;
