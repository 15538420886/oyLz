...
var UpdateArticlesPage = React.createClass({
	...
	initPage: function(articles)
	{
		this.state.hints = {};
		Utils.copyValue(articles, this.state.articles);
	...
	onClickSave : function(){
		if(Common.formValidator(this, this.state.articles)){
			...
			ArticlesActions.updateArticles( obj );
		}
	},
	...
	render : function() {
		...
		return (
			<Modal visible={this.state.modal} width='540px' title="修改物品清单" maskClosable={false} onOk={this.onClickSave} onCancel={this.toggle}
			  footer={[
			  	<div key="footerDiv" style={{display:'block', textAlign:'right'}}>
					<ServiceMsg ref='mxgBox' svcList={['articles/update']}/>
                    ...
                    <Row>
                        <Col span={12}>
                            <FormItem {...formItemLayout2} label="序号" required={true} colon={true} className={layoutItem} help={hints.inventCodeHint} validateStatus={hints.inventCodeStatus}>
                                <Input type="text" name="inventCode" id="inventCode" value={this.state.articles.inventCode } disabled={true} onChange={this.handleOnChange} />
                            </FormItem>
                        </Col>
                        <Col span={12}>
