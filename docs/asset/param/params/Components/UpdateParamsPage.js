...
var UpdateParamsPage = React.createClass({
	...
	{
		this.state.hints = {};
		Utils.copyValue(params, this.state.params);
	...
	onClickSave : function(){
		if(Common.formValidator(this, this.state.params)){
			...
			ParamsActions.updateParams( obj );
		}
	},
	...
	render : function() {
		...
		return (
			<Modal visible={this.state.modal} width='540px' title="修改规格参数" maskClosable={false} onOk={this.onClickSave} onCancel={this.toggle}
			  footer={[
			  	<div key="footerDiv" style={{display:'block', textAlign:'right'}}>
					<ServiceMsg ref='mxgBox' svcList={['params/update']}/>
                    ...
                    <Row>
                        <Col span={12}>
							<FormItem {...formItemLayout2} label="名称" required={true} colon={true} className={layoutItem} help={hints.paramNameHint} validateStatus={hints.paramNameStatus}>
								<Input type="text" name="paramName" id="paramName" value={this.state.params.paramName } disabled={true} onChange={this.handleOnChange} />
							</FormItem>
                        </Col>
                    </Row>
