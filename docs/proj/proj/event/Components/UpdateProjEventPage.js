...
var UpdateProjEventPage = React.createClass({
    ...
    componentDidMount: function () {
        ...
        this.initPage(this.props.projEvent);
    },
    componentWillReceiveProps: function (newProps) {
        this.initPage(newProps.projEvent);
    },
    initPage: function (projEvent) {
        Utils.copyValue(projEvent, this.state.projEvent);
        if (this.refs.empSearchBox !== undefined) {
            this.refs.empSearchBox.setValue(projEvent.eventData3);
        }
    ...
    onClickSave: function () {
        if (Common.formValidator(this, this.state.projEvent)) {
            ...
            ProjEventActions.updateProjEvent(this.state.projEvent);
        }
    },
    goBack: function () {
        this.props.onBack();
        ...
        return (
            <div style={{ overflow: 'hidden', height: '100%', paddingLeft: '4px' }}>
                <Tabs defaultActiveKey="2" onChange={this.onTabChange} tabBarStyle={{ paddingLeft: '16px', margin: '-36px 0 0' }} style={{ width: '100%', height: '100%', padding: '36px 0 0' }}>
                    ...
                    <TabPane tab="修改项目组事件" key="2" style={{ width: '100%', height: '100%' }}>
                        <div style={{ padding: "20px 0 16px 0px", height: '100%', overflowY: 'auto' }}>
                            <ServiceMsg ref='mxgBox' svcList={['proj-event/update']} />
                                ...
                                <Row>
                                    <Col span="12">
                                        <FormItem {...formItemLayout2} label="发生日期" required={true} colon={true} className={layoutItem} help={hints.eventDateHint} validateStatus={hints.eventDateStatus}>
                                            <DatePicker style={{ width: '100%' }} name="eventDate" id="eventDate" value={this.formatDate(this.state.projEvent.eventDate, Common.dateFormat)} format={Common.dateFormat} onChange={this.handleOnSelDate.bind(this, "eventDate", Common.dateFormat)} />
                                        </FormItem>
                                    </Col>
                                    <Col span="12">
...
export default UpdateProjEventPage;
