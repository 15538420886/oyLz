	      	...
	      	recordSet.map((pool, i) => {
				return <div key={pool.uuid} className='card-div' style={{width: 300}}>
					<div className="ant-card ant-card-bordered" style={{width: '100%'}} >
						...
						<div className="ant-card-body" onClick={this.handleCorpClick.bind(this, pool)} title='点击进入公司维护页面' style={{cursor:'pointer', height:'66px', overflow:'hidden'}}>{pool.poolDesc}</div>
					</div>
				</div>
	      	});
