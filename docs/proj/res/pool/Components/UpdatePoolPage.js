...
var UpdatePoolPage = React.createClass({
	...
	componentDidMount : function(){
		this.state.validRules = [
			...
			{id: 'poolName', desc:'资源池名称', required: false, max: '64'},
			{id: 'poolCode', desc:'资源池编号', required: false, max: '64'},
			{id: 'poolDesc', desc:'资源池说明', required: false, max: '256'},
		];
		this.initPage( this.props.pool );
	},

	initPage: function(pool)
	{
		Utils.copyValue(pool, this.state.pool);

		if (this.refs.empSearchBox !== undefined) {
            this.refs.empSearchBox.setValue(pool.pmName);
        }
	...
	onClickSave : function(){
		if(Common.formValidator(this, this.state.pool)){
			...
			PoolActions.updateResPool( this.state.pool );
		}
	},
		...
		return (
			<div style={{overflow:'hidden', height:'100%', paddingLeft: '4px'}}>
				<Tabs defaultActiveKey="2"  onChange={this.onTabChange} tabBarStyle={{paddingLeft: '16px', margin: '-36px 0 0'}} style={{width: '100%', height: '100%', padding: '36px 0 0'}}>
					...
					<TabPane tab="修改资源池" key="2" style={{width: '100%', height: '100%'}}>
                        <div style={{padding:"20px 0 16px 0px", height: '100%',overflowY: 'auto'}}>
			            	<ServiceMsg ref='mxgBox' svcList={['res-pool/update']}/>
                           ...
                           <Form layout={layout} style={{width:'450px'}}>
								...
								<FormItem {...formItemLayout} label="名称" required={false} colon={true} className={layoutItem} help={hints.poolNameHint} validateStatus={hints.poolNameStatus}>
