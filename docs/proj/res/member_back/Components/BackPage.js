...
var BackPage = React.createClass({
	...
	mixins: [Reflux.listenTo(MemberBackStore, "onServiceComplete"), ModalForm('memberBack'),ProjCodeMap()],
	...
	componentDidMount : function(){
		this.state.validRules = [
            ...
            {id: 'beginHour', desc: '入组时间', required: true, max: '24', pattern: /^(([0-1][0-9])|(2[0-3])):([0-5][0-9])$/, patternPrompt: '时间格式错误' },
		];
	...
	onClickSave : function(){
		if(Common.formValidator(this, this.state.memberBack)){
			this.state.memberBackSet.operation = '';
			this.setState({loading: true});
			MemberBackActions.createResMember( this.state.memberBack );
		}
	},
	goBack:function(){
        this.props.onBack();
	    ...
	    return (
			<div style={{overflow:'hidden', height:'100%', paddingLeft: '4px'}}>
				<Tabs defaultActiveKey="2"  onChange={this.onTabChange} tabBarStyle={{paddingLeft: '16px', margin: '-36px 0 0'}} style={{width: '100%', height: '100%', padding: '36px 0 0'}}>
					...
					<TabPane tab="人员回组" key="2" style={{width: '100%', height: '100%',overflowY:'auto'}}>
