...
var BatchCreateResMemberPage = React.createClass({
    ...
    uploadComplete: function (errMsg, result) {
        ...
        if (errMsg !== '') {
            ...
            for (var i = 0; i < result.list.length; i++) {
                ...
                item.workBegin = ProjContext.calcBeginMonth(item.entryDate, item.workYears);
                item.induBegin = ProjContext.calcBeginMonth(item.entryDate, item.induYears);
                ...
                item.userUuid = item.uuid;
                item.manType = (item.empType == '员工') ? '员工' : '实习';
                item.status = '在岗';
    ...
    beforeUpload: function (file) {
        this.setState({ loading: true });
        var url = Utils.hrUrl + 'hr-employee/retrieve-xls';
...
module.exports = BatchCreateResMemberPage;
