...
var UpdateResRolePage = React.createClass({
	...
	componentDidMount : function(){
		...
		this.initPage( this.props.resRole );
	},
	
	initPage: function(resRole)
	{
		Utils.copyValue(resRole, this.state.resRole);
		
		if (this.refs.empSearchBox !== undefined) {
            this.refs.empSearchBox.setValue(resRole.perName);
        }
	...
	onClickSave : function(){
		if(Common.formValidator(this, this.state.resRole)){
			this.state.resRoleSet.operation = '';
			this.setState({loading: true});
			ResRoleActions.updateResRole( this.state.resRole );
		}
	},
				...
				<Tabs defaultActiveKey="2"  onChange={this.onTabChange} tabBarStyle={{paddingLeft: '16px', margin: '-36px 0 0'}} style={{width: '100%', height: '100%', padding: '36px 0 0'}}>
					...
					<TabPane tab="修改管理员" key="2" style={{width: '100%', height: '100%'}}>
                        <div style={{padding:"20px 0 16px 0px", height: '100%',overflowY: 'auto'}}>
			            	<ServiceMsg ref='mxgBox' svcList={['res-role/update']}/>
...
export default UpdateResRolePage;
// poolUuid={poolUuid} 
