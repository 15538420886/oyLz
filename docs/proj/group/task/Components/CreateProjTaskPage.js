...
import SearchProjTask from '../../../lib/Components/SearchProjTask';

var CreateProjTaskPage = React.createClass({
    ...
    clear : function(){
        this.state.hints = {};
        this.state.projTask.uuid='';
        this.state.projTask.projUuid = this.props.projUuid;
        this.state.projTask.corpUuid = window.loginData.compUser.corpUuid;
        
        this.state.projTask.ordCode='';
        this.state.projTask.ordName='';
        this.state.projTask.itemCode='';
        this.state.projTask.itemName='';
        this.state.projTask.beginDate='';
        this.state.projTask.endDate='';
        this.state.projTask.tastStatus='有效任务';
    ...
    onClickSave : function(){
        if(Common.formValidator(this, this.state.projTask)){
            ...
            ProjTaskActions.createProjTask( this.state.projTask );
        }
    },

    onSelectProjTask:function(data){
        this.state.projTask.ordUuid = data.uuid;
        this.state.projTask.ordCode = data.ordCode;
        this.state.projTask.ordName = data.ordName;
        this.state.projTask.itemUuid = data.itemUuid;
		this.state.projTask.itemCode = data.itemCode;
        this.state.projTask.itemName = data.itemName;
    ...
    render : function(){
        ...
        var grpUuid = this.props.parentUuid;
        return (
            <div style={{overflow:'hidden', height:'100%', paddingLeft: '4px'}}>
                <Tabs defaultActiveKey="2"  onChange={this.onTabChange} tabBarStyle={{paddingLeft: '16px', margin: '-36px 0 0'}} style={{width: '100%', height: '100%', padding: '36px 0 0'}}>
                    <TabPane tab="返回" key="1" style={{width: '100%', height: '100%'}}>
                    </TabPane>
                    <TabPane tab="增加任务" key="2" style={{width: '100%', height: '100%'}}>
                        <div style={{padding:"24px 0 16px 8px", height: '100%',overflowY: 'auto'}}>
                            <ServiceMsg ref='mxgBox' svcList={['proj_task/create']}/>
                            <Form layout={layout} style={{width:'600px'}}>
                                <Row>
                                    <FormItem {...formItemLayout} className={layoutItem} label='任务查询' colon={true} help={hints.ordCodeHint} validateStatus={hints.ordCodeStatus}>
                                        <SearchProjTask grpUuid={grpUuid} showError={this.showError} onSelectProjTask={this.onSelectProjTask}/>
                                    </FormItem>
                                </Row>
                                <Row>
									<Col span="12">
...
export default CreateProjTaskPage;