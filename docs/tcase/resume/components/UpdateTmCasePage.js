...
import { Form, Modal, Button, Upload,Input, Select, Spin, AutoComplete,Row, Col,message,Table,Popconfirm ,Icon} from 'antd';
...
var UpdateTmCasePage = React.createClass({
...
//      }
    ...
    componentDidMount: function () {
       this.state.validRules = [
	        ...
	        {id: 'caseCode', desc:'用例编码', required: true, max: '255'},
    ...
    onClickSave: function () {
        if (Validator.validator(this, this.state.caselist)) {
	        console.log()
             QueryActions.updateTmCase(this.state.caselist,this.props.text,this.state.caselistSet.startPage,this.state.caselistSet.pageRow)
//	       
//          this.state.caselistSet.operation = '';
    ...
    handleAdd:function(){
    	   ...
    	   if(this.state.caselist.caseSteps == null){
    ...
    render: function () {
        ...
        console.log(this.state.caselist)
         const props = {
			name: 'file',
			multiple: true,
			action: 'http://10.10.10.201:8082/defect_s/bug-attach-upload/upload?uuid='+this.state.caselist.uuid,
			onChange(info) {
...
export default UpdateTmCasePage;
